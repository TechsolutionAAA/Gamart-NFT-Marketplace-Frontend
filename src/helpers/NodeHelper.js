import { minutesAgo } from "./index.js";
import { EnvHelper } from "./Environment.js";
...
export class NodeHelper {
   ...
   */
  static _emptyInvalidNodesList(networkId) {
  ...
  static _removeNodeFromProviders(providerKey, providerUrl, networkId) {
   ...
   */
  static logBadConnectionWithTimer(providerUrl, networkId) {
   ...
   * @returns string
   */
  static getMainnetURI = (networkId) => {
  ...
  static getMainnetStaticProvider = () => {
    return new StaticJsonRpcProvider(EnvHelper.bscNetURI);
  };

  /**
   * returns Array of APIURIs where NOT on invalidNodes list
   */
  static getNodesUris = (networkId) => {
   ...
   */
  static checkAllNodesStatus = async (networkId) => {
   ...
   */
  static checkNodeStatus = async (url, networkId) => {
   ...
   */
  static validityCheck = ({ nodeMethod, resultVal }) => {
    switch (nodeMethod) {
      case "net_peerCount":
